...
Use this code to reproduce Figure 5 from Bagnoli et al., 2017.

Step 1: load required packages & functions:

``` r
# Load Packages & Functions -----------------------------------------------
pckgs <- c("here","ggplot2","dplyr","cowplot","MASS")
...
Step 2: load and process the Svensson et al., 2017 data

``` r
#here, we used the published molecular detection limits from the supplement
svensson <- read.csv(paste(path_to_data,"Data/nmeth.4220-S3.csv",sep="/"),stringsAsFactors = F)
...
Step 3: load and process the mcSCRB-seq data

``` r
J1_ERCC_all <- readRDS(paste(path_to_data,"Data/J1_ERCC.rds",sep="/"))
...
ERCCannot <- read.table(paste(path_to_data,"Data/ERCC_molecules_J1.txt",sep="/"),header=T,stringsAsFactors = F)
...
Step 4: load and process the Sasagawa et al., 2017 (Quartz-seq2) data

``` r
#downloaded mESC plate containing ERCCs (GSM2656466)
quartzseq <- read.table(paste(path_to_data,"Data/Quartzseq2.GSM2656466_ESPrE_plate6_RT25_384well.txt",sep="/"),header=T,row.names=1)
quartzseq <- quartzseq[grep("ERCC",row.names(quartzseq)),]

#read in the general ERCC annotation
ercc_annot <- read.table(paste(path_to_data,"Data/ERCC_annotation.txt",sep="/"), header=T, stringsAsFactors = F,sep="\t")
...
Step 5: load and process the Ziegenhain et al., 2017 data

``` r
#here we use the published data from GEO accession GSE75790
ziegenhain <-  read.table(paste(path_to_data,"Data/GSE75790_ziegenhain_complete_data.txt",sep="/"), header=T,row.names=1)
...
ercc.molecules.z <- read.table(file=paste(path_to_data,"Data/ERCC_molecules_Ziegenhain2017.txt",sep="/"), header=T, sep="\t", stringsAsFactors = F)
...
Step 6: load and process the Chromium (Zheng et al., 2017) data

``` r
#data obtained from https://support.10xgenomics.com/single-cell-gene-expression/datasets/1.1.0/ercc
ERCC10x <- read.table(paste(path_to_data,"Data/ERCCs10x_matrix.mtx",sep="/"), header = F,stringsAsFactors = F,skip = 3)
...
Step 7: merge all the data and plot
...
![](Figure5_ERCC_Notebook_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-7-1.png)

Step 8: Example plot for binomial logistic regression
...
![](Figure5_ERCC_Notebook_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-8-1.png)

Step 9: mcSCRB-seq detection limit vs sequencing
...
![](Figure5_ERCC_Notebook_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-9-1.png)

Step 10: Save final plot

``` r
p_Fig5 <- plot_grid(plot_grid(p_Fig5a,p_Fig5b,nrow = 2,labels = c("A","B"),align = "hv"),p_Fig5c,ncol = 2,labels = c("","C"))
ggsave(p_Fig5,filename = paste(path_to_data,"PDF_output","Figure5.pdf",sep="/"),device="pdf",width = 15,height = 12)
```
